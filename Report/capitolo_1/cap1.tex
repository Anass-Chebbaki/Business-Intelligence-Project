...
\end{enumerate}
\vspace{2mm}
Il file \texttt{FAOSTAT\_data\_11-24-2020.csv} fornisce una classificazione dettagliata dei paesi e delle regioni del mondo attraverso l’utilizzo di codici standardizzati, rendendolo una risorsa fondamentale per l’integrazione e l’armonizzazione di dati geografici in diversi contesti analitici. Nell'elenco \ref{elenco_dati2} sono riportati i campi presenti in questo file:
\vspace{2mm}
\begin{enumerate}
\label{elenco_dati2}
    \item \texttt{Country Code}: un codice numerico univoco assegnato a ciascun paese o entità geografica per facilitarne l’identificazione.
    \item \texttt{Country}: il nome del paese o della regione corrispondente al codice numerico. Questo campo può includere sia stati sovrani che entità territoriali specifiche.
    \item \texttt{M49 Code}: un codice numerico definito dalle Nazioni Unite per la classificazione statistica delle regioni e dei paesi. Questo sistema è ampiamente utilizzato negli studi internazionali.
    \item \texttt{ISO2 Code}:  il codice a due lettere assegnato da ISO (International Organization for Standardization), utilizzato nelle abbreviazioni internazionali e nelle applicazioni informatiche.
    \item \texttt{ISO3 Code}: il codice a tre lettere ISO che rappresenta in modo univoco ogni paese, spesso usato in contesti economici e statistici.
    \item \texttt{Start Year}: l'anno in cui il codice o la classificazione del paese è entrata in vigore. Questo campo è utile per tracciare modifiche amministrative o riconoscimenti internazionali.
    \item \texttt{End Year}: l’anno in cui il codice o la classificazione è cessata di essere valida, ad esempio a seguito di cambiamenti territoriali o della fusione di stati.
\end{enumerate}
...
Il dataset è composto da un unico file denominato \texttt{global-data-on-sustainable-energy.csv}. Di seguito, nell'elenco \ref{elenco_dati3}, sono riportati i principali campi presenti all'interno del file:
\vspace{2mm}
\begin{enumerate}
\label{elenco_dati3}
    ...
    \item \texttt{Value\_co2\_emissions (kt)}: emissioni di anidride carbonica dello Stato, espresse in kiloton.
